...
- **Custom AI Provider (OpenAI/Anthropic compatible)**: `ModelConfig.providerId` (default `null`) selects the backend. `null` = Google Agent Platform (original, untouched code path in `agentPlatform.js`). Set = `agentPlatform.js` delegates to `packages/ai-providers/services/agentPlatformBridge.js`, which resolves the `AIProvider` doc (managed at `/admin/providers`, sidebar group "NQDEV Platform", route/view/models all under `packages/ai-providers/`), rotates its `ProviderApiKey` pool via `packages/ai-providers/services/providerKeyManager.js` (mirrors `server/services/apiKeyManager.js` but keyed per-provider), and calls the matching adapter in `packages/ai-providers/services/providerAdapters/{openai,anthropic}.js`.
  ...
  - As of 2026-08-05 (`plans/2026-08-05-kira-provider-full-parity-planning.md`) the architecture supports all 4 categories (`text`/`image`/`tts`/`video`) for OpenAI-compatible providers — `agentPlatformBridge.js` exports `generateTTS`/`generateVideo`/`pollVideo` alongside the original `generateText(Stream)`/`generateImage`, and `agentPlatform.js` has a `model.providerId` branch at the top of `generateTTS()`, `initiateVideo()` (placed **before** the `apiKeyInfo.projectNumber` check — that check is Google-only and would wrongly reject custom-provider requests if placed first), and `pollVideo()`. Anthropic's adapter still only implements `generateText(Stream)` (no image/tts/video API), so `type: 'anthropic'` providers remain text-only in practice — this is a per-adapter capability gap, not an architectural restriction. Per-provider seed data still decides what's actually offered: `9router` (`server/seed.js`) seeds `text` only; `kira` seeds all 4. TTS/video adapter calls for Kira (`providerAdapters/openai.js`'s `generateTTS`/`generateVideo`/`pollVideoOperation`) are implemented against the documented OpenAI-shape contract but **not yet verified against a real `KIRA_API_KEY`** (no key was available at implementation time) — treat response-shape assumptions there (binary `/audio/speech`, `{id}`-style `/videos/generations` submit, `{status,data:[{url}]}`-style `/videos/operations/:uuid` poll) as unverified until tested live.
